...
class TestCreateCourier:
    ...
    @allure.description("Проверяем успешное создание нового курьера с валидными данными.")
    def test_create_courier_success(self, login_courier, cleanup_courier):
        body = CourierFactory.default_body_with_random_parameters()
        create_courier_response = CourierApi.create_courier(body)
        cleanup_courier.append(login_courier(body).json()["id"])
    ...
    def test_cannot_create_duplicate_courier(self, create_courier, login_courier, cleanup_courier):
        body = CourierFactory.default_body_with_random_parameters()
        create_courier(body)
        create_courier_response = CourierApi.create_courier(body)
        cleanup_courier.append(login_courier(body).json()["id"])
